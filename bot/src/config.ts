...
const envSchema = z.object({
  ...
  VPN_XUI_MULTI_SUB_BASE_URL: z.union([z.string().url(), z.literal("")]).default(""),
  ...
  QWEN_TRANSCRIPTION_MODEL: z.string().min(1).default("qwen3-asr-flash"),
  QWEN_TRANSCRIPTION_LANGUAGE: z.string().optional().default("ru"),
  QWEN_TRANSCRIPTION_CHUNK_SECONDS: z.coerce.number().int().positive().max(300).default(290),
  QWEN_TRANSCRIPTION_MAX_FILE_SIZE_MB: z.coerce.number().int().positive().max(7).default(7),
...
export const config = {
  ...
  media: {
    ...
    qwenTranscriptionModel: env.QWEN_TRANSCRIPTION_MODEL,
    qwenTranscriptionLanguage: env.QWEN_TRANSCRIPTION_LANGUAGE,
    qwenTranscriptionChunkSeconds: env.QWEN_TRANSCRIPTION_CHUNK_SECONDS,
    qwenTranscriptionMaxFileSizeMb: env.QWEN_TRANSCRIPTION_MAX_FILE_SIZE_MB,
    qwenTranscriptionTimeoutMs: env.QWEN_TRANSCRIPTION_TIMEOUT_MS,
    qwenTranscriptCleanupModel: env.QWEN_TRANSCRIPT_CLEANUP_MODEL,
    qwenTranscriptCleanupTimeoutMs: env.QWEN_TRANSCRIPT_CLEANUP_TIMEOUT_MS,
  },
  ...
  vpnAccessSync: {
    ...
    paidWhitelistTrafficLimitBytes: BigInt(env.VPN_PAID_WHITELIST_TRAFFIC_LIMIT_GB) * 1024n ** 3n,
    paidWhitelistTrafficResetDays: env.VPN_PAID_WHITELIST_TRAFFIC_RESET_DAYS,
  },
} as const;
