...
| `/ask <вопрос>` | USER, ADMIN | Найти опыт участников в истории клуба |
...
## AI-поиск по истории клуба

Поиск запускается только явной командой `/ask <вопрос>` в личном чате с ботом
и доступен участникам клуба после обычной проверки членства. Обычные сообщения
и сообщения в группе не запускают агента.

Бот ищет кандидатов только среди текстов и подписей к сообщениям из
`CLUB_GROUP_ID`, передаёт модели не более `AI_SEARCH_MAX_RESULTS` коротких
фрагментов и принимает структурированный ответ с ID использованных источников.
Эти ID проверяются по фактической выборке, а Telegram-ссылки формируются
приложением. Полная история клуба в один запрос модели не отправляется.
